...
from muecke.core.signals import order_submitted
...
from muecke.payment.settings import PM_ORDER_IMMEDIATELY
from muecke.payment.settings import PM_ORDER_ACCEPTED
...
def process_payment(request):
    ...
    if payment_method.module:
        payment_class = muecke.core.utils.import_symbol(payment_method.module)
        ...
        if create_order_time == PM_ORDER_IMMEDIATELY:
            order = muecke.order.utils.add_order(request)
        ...
        return {
            "accepted": True,
            "next_url": reverse("muecke_thank_you"),
        }
    else:
        order = muecke.order.utils.add_order(request)
        order_submitted.send({"order": order, "request": request})
