...
def get_current_product_category(request, product):
    ...
    """
    logger.info("Decprecated: muecke.catalog.utils: the function 'get_current_product_category' is deprecated. Please use 'get_current_category'of the Product class.")
...
def get_property_groups(category):
    ...
    """
    logger.info("Decprecated: muecke.catalog.utils: the function 'get_property_groups' is deprecated. Please use 'get_property_groups'of the Category class.")
...
def get_price_filters(category, product_filter, price_filter):
    ...
    if price_filter:
        ...
        products = muecke.catalog.models.Product.objects.filter(
    ...
    for n, i in enumerate(range(0, int(pmax), step)):
        ...
        min = i + 1
        max = i + step
        products = muecke.catalog.models.Product.objects.filter(effective_price__range=(min, max), pk__in=product_ids)
        result.append({
            "min": min,
            "max": max,
...
def get_filtered_products_for_category(category, filters, price_filter, sorting):
    ...
    if filters:
        ...
        all_variants = muecke.catalog.models.Product.objects.filter(parent__in=products)
        ...
        products = muecke.catalog.models.Product.objects.filter(pk__in=matched_product_ids).distinct()
    else:
        categories = [category]
        if category.show_all_products:
    ...
    if price_filter:
        ...
        variants = muecke.catalog.models.Product.objects.filter(parent__in=products)
        ...
        products = muecke.catalog.models.Product.objects.filter(pk__in=matched_product_ids)
...
def get_option_mapping():
    ...
    """
    options = {}
    for option in muecke.catalog.models.PropertyOption.objects.all():
...
def get_property_mapping():
    ...
    """
    properties = {}
    for property in muecke.catalog.models.Property.objects.all():
...
def _calculate_steps(product_ids, property, min, max):
    ...
    **Parameters**
    ...
    filter_steps = muecke.catalog.models.FilterStep.objects.filter(property=property.id)
...
def calculate_packages(product, quantity):
    ...
    DEPRECATED.
    """
    logger.info("Decprecated: muecke.catalog.utils: the function 'calculate_packages' is deprecated.")
...
def calculate_real_amount(product, quantity):
    ...
    """
    logger.info("Decprecated: muecke.catalog.utils: the function 'calculate_real_amount' is deprecated. Please use 'get_amount_by_packages'of the Product class.")
