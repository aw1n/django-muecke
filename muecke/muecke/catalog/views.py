...
from muecke.catalog.models import Category, Property
...
from muecke.catalog.settings import SELECT
from muecke.core.utils import LazyEncoder, muecke_pagination
from muecke.core.templatetags import muecke_tags
from muecke.utils import misc as muecke_utils
...
def calculate_packing(request, id, quantity=None, with_properties=False, as_string=False, template_name="muecke/catalog/packing_result.html"):
...
def set_filter(request, category_slug, property_id, value=None, min=None, max=None):
    ...
    url = reverse("muecke_category", kwargs={"slug": category_slug})
...
def set_sorting(request):
    ...
    # muecke_sorting_changed.send(category_id)
    return HttpResponseRedirect(request.META.get("HTTP_REFERER"))


def category_view(request, slug, template_name="muecke/catalog/category_base.html"):
    """
    """
    start = request.REQUEST.get("start", 1)
    category = muecke_get_object_or_404(Category, slug=slug)
    ...
    return render_to_response(template_name, RequestContext(request, {
        ...
        "top_category": muecke.catalog.utils.get_current_top_category(request, category),
    }))


def category_categories(request, slug, start=0, template_name="muecke/catalog/categories/category/default.html"):
...
def category_products(request, slug, start=1, template_name="muecke/catalog/categories/product/default.html"):
    ...
    all_products = muecke.catalog.utils.get_filtered_products_for_category(
    ...
    # Calculate urls
    pagination_data = muecke_pagination(request, current_page, url=category.get_absolute_url())
...
def product_view(request, slug, template_name="muecke/catalog/product_base.html"):
    """Main view to display a product.
    """
    product = muecke_get_object_or_404(Product, slug=slug)
...
def product_inline(request, product, template_name="muecke/catalog/products/product_inline.html"):
    """
