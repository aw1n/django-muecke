...
from muecke import shipping
from muecke.caching.utils import muecke_get_object_or_404
from muecke.core.models import Shop, Country
from muecke.criteria.models.criteria_objects import CriteriaObjects
from muecke.criteria.settings import EQUAL
from muecke.criteria.settings import LESS_THAN
from muecke.criteria.settings import LESS_THAN_EQUAL
from muecke.criteria.settings import GREATER_THAN
from muecke.criteria.settings import GREATER_THAN_EQUAL
from muecke.criteria.settings import NUMBER_OPERATORS
from muecke.criteria.settings import SELECT_OPERATORS
from muecke.criteria.settings import IS, IS_NOT, IS_VALID, IS_NOT_VALID
from muecke.payment.models import PaymentMethod
from muecke.shipping.models import ShippingMethod


class Criterion(object):
    """Base class for all muecke criteria.
    """
    class Meta:
        app_label = "criteria"
...
class PaymentMethodCriterion(models.Model, Criterion):
    ...
    def is_valid(self, request, product=None):
        ...
        if not is_payment_method and self.operator == IS:
            payment_method = muecke.payment.utils.get_selected_payment_method(request)
            ...
            for pm in self.payment_methods.all():
                if not muecke.criteria.utils.is_valid(request, pm, product):
                    return False
            return True
        elif self.operator == IS_NOT_VALID:
            for pm in self.payment_methods.all():
                if muecke.criteria.utils.is_valid(request, pm, product):
                    return False
            return True
        else:
            return False
...
class ShippingMethodCriterion(models.Model, Criterion):
    ...
    def is_valid(self, request, product=None):
        ...
        if not is_shipping_method and self.operator == IS:
            shipping_method = muecke.shipping.utils.get_selected_shipping_method(request)
            ...
            for sm in self.shipping_methods.all():
                if not muecke.criteria.utils.is_valid(request, sm, product):
                    return False
            return True
        elif self.operator == IS_NOT_VALID:
            for sm in self.shipping_methods.all():
                if muecke.criteria.utils.is_valid(request, sm, product):
                    return False
            return True
        else:
            return False
...
class DistanceCriterion(models.Model, Criterion):
    ...
    def is_valid(self, request, product=None):
        ...
        """
        try:
            m = muecke.core.utils.import_module(settings.LFS_DISTANCE_MODULE)
