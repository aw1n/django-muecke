...
class Criterion(models.Model):
    ...
    **Constants:**
    ...
    def cart(self):
        ...
        """
        return muecke.cart.utils.get_cart(self.request)
    ...
    def is_valid(self):
        ...
        if (not is_payment_method) and (self.operator == self.IS_SELECTED):
            ...
            for pm in self.value.all():
                if not muecke.criteria.utils.is_valid(self.request, pm, self.product):
                    return False
            return True
        elif self.operator == self.IS_NOT_VALID:
            for pm in self.value.all():
                if muecke.criteria.utils.is_valid(self.request, pm, self.product):
                    return False
            return True
        else:
            return False
    ...
    def is_valid(self):
        ...
        if (not is_shipping_method) and (self.operator == self.IS_SELECTED):
            ...
            for sm in self.value.all():
                if not muecke.criteria.utils.is_valid(self.request, sm, self.product):
                    return False
            return True
        elif self.operator == self.IS_NOT_VALID:
            for sm in self.value.all():
                if muecke.criteria.utils.is_valid(self.request, sm, self.product):
                    return False
            return True
        else:
            return False
