...
class FilterPortlet(Portlet):
    ...
    def render(self, context):
        ...
        if category is None:
            return render_to_string("muecke/portlets/filter.html", {
                "show": False,
            })
        ...
        if self.show_product_filters:
            product_filters = muecke.catalog.utils.get_product_filters(category,
        ...
        if self.show_price_filters:
            price_filters = muecke.catalog.utils.get_price_filters(category,
                set_product_filters, set_price_filters)
        else:
            price_filters = None

        return render_to_string("muecke/portlets/filter.html", RequestContext(request, {
            "show": True,
            "title": self.title,
            "category": category,
