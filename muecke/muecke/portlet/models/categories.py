...
class CategoriesPortlet(Portlet):
    ...
    def render(self, context):
        ...
        current_categories = muecke.core.utils.get_current_categories(request, object)

        ct = muecke.core.utils.CategoryTree(
            current_categories, self.start_level, self.expand_level)
        category_tree = ct.get_category_tree()

        result = render_to_string("muecke/portlets/categories.html", RequestContext(request, {
            "title": self.title,
