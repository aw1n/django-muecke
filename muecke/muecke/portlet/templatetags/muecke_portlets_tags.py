...
from muecke.portlet.models import RecentProductsPortlet
from muecke.portlet.models import RelatedProductsPortlet
from muecke.portlet.models import TopsellerPortlet
...
def muecke_portlet_slot(context, slot_name):
    ...
    instance = context.get("category") or \
               ...
               context.get("page") or \
               muecke.core.utils.get_default_shop(request)

    cache_key = "%s-muecke-portlet-slot-%s-%s-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, slot_name, instance.__class__.__name__, instance.id)
...
@register.inclusion_tag('muecke/portlets/portlet.html', takes_context=True)
def muecke_cart_portlet(context, title=None):
...
@register.inclusion_tag('muecke/portlets/portlet.html', takes_context=True)
def muecke_categories_portlet(context, title=None):
...
@register.inclusion_tag('muecke/portlets/portlet.html', takes_context=True)
def muecke_pages_portlet(context, title=None):
...
@register.inclusion_tag('muecke/portlets/portlet.html', takes_context=True)
def muecke_recent_products_portlet(context, title=None):
...
@register.inclusion_tag('muecke/portlets/portlet.html', takes_context=True)
def muecke_related_products_portlet(context, title=None):
    """Tag to render the related products portlet.
    """
    if title is None:
...
@register.inclusion_tag('muecke/portlets/portlet.html', takes_context=True)
def muecke_topseller_portlet(context, title=None, limit=5):
    """Tag to render the related products portlet.
    """
    if title is None:
        title = _(u"Topseller")
