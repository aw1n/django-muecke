...
from muecke.marketing.models import Topseller
from muecke.marketing.utils import calculate_product_sales
from muecke.order.models import Order
from muecke.order.models import OrderItem
from muecke.order.settings import CLOSED
...
class RatingMailTestCase(TestCase):
    ...
    def test_get_orders(self):
        ...
        orders = muecke.marketing.utils.get_orders()
...
class TopsellerUtilsTestCase(TestCase):
    """Tests the utils of the muecke.marketing
    """
    def setUp(self):
        """
        """
    ...
    def test_topseller_1(self):
        ...
        """
        ts = muecke.marketing.utils.get_topseller(2)
    ...
    def test_topseller_for_category_1(self):
        ...
        ts = muecke.marketing.utils.get_topseller_for_category(self.c1, limit=2)
        self.assertEqual(len(ts), 2)
        ...
        ts = muecke.marketing.utils.get_topseller_for_category(self.c11, limit=1)
        self.assertEqual(len(ts), 1)

        self.assertEqual(ts[0], self.p2)

        ts = muecke.marketing.utils.get_topseller_for_category(self.c12, limit=1)
        self.assertEqual(len(ts), 1)
    ...
    def test_topseller_for_category_2(self):
        ...
        # Tests the direct categories
        ts = muecke.marketing.utils.get_topseller_for_category(self.c11, limit=2)
        self.assertEqual(len(ts), 2)
        self.assertEqual(ts[0], self.p5)
        ...
        # c2
        ts = muecke.marketing.utils.get_topseller_for_category(self.c12, limit=2)
        self.assertEqual(len(ts), 2)
        self.assertEqual(ts[0], self.p4)
        self.assertEqual(ts[1], self.p3)
