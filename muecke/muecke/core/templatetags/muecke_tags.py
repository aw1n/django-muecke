...
from muecke.catalog.settings import SORTING_MAP
from muecke.catalog.models import Product
from muecke.catalog.models import PropertyOption
from muecke.catalog.settings import PRODUCT_TYPE_LOOKUP
import muecke.core.utils
from muecke.core.models import Action
from muecke.page.models import Page
from muecke.shipping import utils as shipping_utils

register = template.Library()


@register.inclusion_tag('muecke/portlets/category_children.html', takes_context=True)
def category_children(context, categories):
    """
    """
    return {"categories": categories}

#@register.inclusion_tag('muecke/shop/tabs.html', takes_context=True)
#def category_tab(context):
#    request = context.get("request")
#    categories = []
#    
#    for categories in Category.objects.filter(parent=None):
#        categories.append({
#		"url": category.get_absolute_url(),
#		"name": category.name})
#    return {"categories": categories}

@register.inclusion_tag('muecke/shop/google_analytics_tracking.html', takes_context=True)
def google_analytics_tracking(context):
    """Returns google analytics tracking code which has been entered to the
    shop.
    """
    shop = muecke.core.utils.get_default_shop(context.get("request"))
    return {
...
@register.inclusion_tag('muecke/shop/google_analytics_ecommerce.html', takes_context=True)
...
@register.inclusion_tag('muecke/shipping/shipping_tag.html', takes_context=True)
def shipping(context, variant):
    return _get_shipping(context, variant)


@register.inclusion_tag('muecke/catalog/sorting.html', takes_context=True)
def sorting(context):
    """
    """
    request = context.get("request")
...
@register.inclusion_tag('muecke/catalog/breadcrumbs.html', takes_context=True)
...
@register.inclusion_tag('muecke/catalog/product_navigation.html', takes_context=True)
...
def tabs(context, obj=None):
    ...
    if isinstance(obj, (Product, Category)):
        top_category = muecke.catalog.utils.get_current_top_category(request, obj)
        if top_category:
            for tab in tabs:
	...
	categories.append({"url":category.get_absolute_url(), "name":category.name,})	
...
@register.inclusion_tag('muecke/catalog/top_level_categories.html', takes_context=True)
...
class TopLevelCategory(Node):
    ...
    def render(self, context):
        ...
        top_level_category = muecke.catalog.utils.get_current_top_category(request, obj)
...
class CartInformationNode(Node):
    ...
    def render(self, context):
        request = context.get("request")
        cart = muecke.cart.utils.get_cart(request)
        if cart is None:
            amount_of_items = 0
            price = 0.0
        else:
