...
from muecke.manage.property_groups.forms import PropertyGroupForm
...
def manage_property_groups(request):
    ...
    try:
        ...
        url = reverse("muecke_manage_property_group", kwargs={"id": property.id})
    except IndexError:
        url = reverse("muecke_manage_no_property_groups")
...
def delete_property_group(request, id):
    ...
    return muecke.core.utils.set_message_cookie(
        url=reverse("muecke_manage_property_groups"),
...
def products_inline(request, product_group_id, as_string=False,
    ...
    if category_filter:
        if category_filter == "None":
            ...
            category = muecke_get_object_or_404(Category, pk=category_filter)
            categories = [category]
