...
from muecke.manage.shipping_methods.forms import ShippingMethodAddForm
from muecke.manage.shipping_methods.forms import ShippingMethodForm
...
def manage_shipping(request):
    ...
    try:
        ...
        url = reverse("muecke_manage_no_shipping_methods")
    else:
        url = reverse("muecke_manage_shipping_method",
...
def add_shipping_method(request,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return muecke.core.utils.set_message_cookie(
                url=reverse("muecke_manage_shipping_method", kwargs={"shipping_method_id": new_shipping_method.id}),
    ...
    return render_to_response(template_name, RequestContext(request, {
        "form": form,
        "came_from": request.REQUEST.get("came_from", reverse("muecke_manage_shipping")),
    }))
...
def save_shipping_method_criteria(request, shipping_method_id):
    ...
    """
    shipping_method = muecke_get_object_or_404(ShippingMethod, pk=shipping_method_id)
...
def delete_shipping_method(request, shipping_method_id):
    ...
    return muecke.core.utils.set_message_cookie(
        url=reverse("muecke_manage_shipping"),
