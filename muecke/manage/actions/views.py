...
from muecke.manage.actions.forms import ActionForm
from muecke.manage.actions.forms import ActionAddForm
...
def manage_actions(request):
    ...
    try:
        ...
        url = reverse("muecke_manage_action", kwargs={"id": action.id})
    except IndexError:
        url = reverse("muecke_no_actions")
...
def add_action(request, template_name="manage/actions/add_action.html"):
    ...
    return render_to_response(template_name, RequestContext(request, {
        "form": form,
        "groups": ActionGroup.objects.all(),
        "came_from": request.REQUEST.get("came_from", reverse("muecke_manage_actions")),
    }))
...
def delete_action(request, id):
    ...
    return muecke.core.utils.set_message_cookie(
        url=reverse("muecke_manage_actions"),
