...
def manage_orders(request, template_name="manage/order/manage_orders.html"):
    ...
    try:
        ...
        return HttpResponseRedirect(reverse("muecke_orders"))
    else:
        return HttpResponseRedirect(
            reverse("muecke_manage_order", kwargs={"order_id": order.id}))
...
def change_order_state(request):
    ...
    if order.state == muecke.order.settings.SENT:
        muecke.core.signals.order_sent.send({"order": order, "request": request})
    if order.state == muecke.order.settings.PAID:
        muecke.core.signals.order_paid.send({"order": order, "request": request})
...
def _get_filtered_orders(order_filters):
    ...
    start = order_filters.get("start", "")
    if start != "":
        s = muecke.core.utils.get_start_day(start)
    else:
        s = datetime.min
    ...
    if end != "":
        e = muecke.core.utils.get_end_day(end)
    else:
        e = datetime.max
