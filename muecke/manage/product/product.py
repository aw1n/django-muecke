...
from muecke.catalog.settings import CHOICES
from muecke.catalog.settings import CHOICES_YES
from muecke.catalog.settings import PRODUCT_TEMPLATES
from muecke.catalog.settings import PRODUCT_TYPE_FORM_CHOICES
from muecke.catalog.settings import VARIANT
from muecke.core.utils import LazyEncoder
from muecke.manage.product.images import manage_images
from muecke.manage.product.seo import manage_seo
from muecke.manage.product.properties import manage_properties
from muecke.manage.product.attachments import manage_attachments
from muecke.manage.views.muecke_portlets import portlets_inline
from muecke.manage.utils import get_current_page
...
class ProductDataForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if self.instance:
            ...
            if redirect_to != "":
                muecke.core.utils.set_redirect_for(self.instance.get_absolute_url(), redirect_to)
            else:
                muecke.core.utils.remove_redirect_for(self.instance.get_absolute_url())
...
def product_data_form(request, product_id, template_name="manage/product/data.html"):
    ...
    return render_to_string(template_name, RequestContext(request, {
        "product": product,
        "form": form,
        "redirect_to": muecke.core.utils.get_redirect_for(product.get_absolute_url()),
    }))
...
def add_product(request, template_name="manage/product/add_product.html"):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            url = reverse("muecke_manage_product", kwargs={"product_id": new_product.id})
    ...
    return render_to_response(template_name, RequestContext(request, {
        "form": form,
        "came_from": request.REQUEST.get("came_from", reverse("muecke_manage_product_dispatcher")),
    }))
...
def change_subtype(request, product_id):
    ...
    return muecke.core.utils.set_message_cookie(
        url=reverse("muecke_manage_product", kwargs={"product_id": product_id}),
...
def product_dispatcher(request):
    ...
    try:
        ...
        url = reverse("muecke_manage_product", kwargs={"product_id": product.id})
    except IndexError:
        url = reverse("muecke_manage_no_products")
...
def product_by_id(request, product_id):
    ...
    """
    product = Product.objects.get(pk=product_id)
    url = reverse("muecke.catalog.views.product_view", kwargs={"slug": product.slug})
    return HttpResponseRedirect(url)
...
def _get_filtered_products(request):
    ...
    if category == "None":
        ...
        category = muecke_get_object_or_404(Category, pk=category)
        categories = [category]
