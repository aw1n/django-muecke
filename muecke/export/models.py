...
class Export(models.Model):
    ...
    def get_absolute_url(self):
        ...
        """
        return reverse(
            "muecke_export", kwargs={"export_id": self.id})

    def get_products(self):
        """Returns selected products. Takes variant options into account.
        """
        import muecke.export.utils
        products = []

        for product in self.products.all():
            if product.is_product_with_variants():
                variants = muecke.export.utils.get_variants(product, self)
                if variants:
...
class Script(models.Model):
    ...
    """
    module = models.CharField(max_length=100, default="muecke.export.generic")
