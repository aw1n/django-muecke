...
import muecke.order.utils
import muecke.payment.settings
import muecke.payment.utils
import muecke.shipping.utils
import muecke.voucher.utils
from muecke.cart import utils as cart_utils
from muecke.checkout.forms import OnePageCheckoutForm
from muecke.checkout.settings import CHECKOUT_TYPE_ANON
from muecke.checkout.settings import CHECKOUT_TYPE_AUTH
from muecke.checkout.settings import INVOICE_PREFIX, SHIPPING_PREFIX
from muecke.customer import utils as customer_utils
from muecke.core.models import Country
from muecke.customer.models import Address
from muecke.customer.models import BankAccount
from muecke.customer.forms import RegisterForm
from muecke.customer.views import address_inline, save_address
from muecke.payment.models import PaymentMethod
from muecke.payment.settings import PAYPAL
from muecke.payment.settings import DIRECT_DEBIT
from muecke.payment.settings import CREDIT_CARD
from muecke.voucher.models import Voucher
from muecke.voucher.settings import MESSAGES

# other imports
from postal.library import form_factory


def login(request, template_name="muecke/checkout/login.html"):
    """Displays a form to login or register/login the user within the check out
    process.

    The form's post request goes to muecke.customer.views.login where all the logic
    ...
    if request.POST.get("action") == "login":
        ...
        if login_form.is_valid():
            ...
            return muecke.core.utils.set_message_cookie(reverse("muecke_checkout"),
...
def empty_page_checkout(request, template_name="muecke/checkout/empty_page_checkout.html"):
    """
    """
    return render_to_response(template_name, RequestContext(request, {
        "shopping_url": reverse("muecke.core.views.shop_view"),
    }))


def thank_you(request, template_name="muecke/checkout/thank_you_page.html"):
...
def payment_inline(request, form, template_name="muecke/checkout/payment_inline.html"):
...
def shipping_inline(request, template_name="muecke/checkout/shipping_inline.html"):
    ...
    """
    selected_shipping_method = muecke.shipping.utils.get_selected_shipping_method(request)
    shipping_methods = muecke.shipping.utils.get_valid_shipping_methods(request)
...
def _save_country(request, customer):
    ...
    if country_iso is not None:
        ...
        muecke.shipping.utils.update_to_valid_shipping_method(request, customer)
        muecke.payment.utils.update_to_valid_payment_method(request, customer)
        customer.save()
