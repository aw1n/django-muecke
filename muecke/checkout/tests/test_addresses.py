...
from muecke.customer.models import Customer, Address
from muecke.order.utils import add_order
from muecke.order.settings import SUBMITTED
from muecke.payment.models import PaymentMethod
from muecke.payment.settings import BY_INVOICE, DIRECT_DEBIT
...
class CheckoutAddressesTestCase(TestCase):
    ...
    def test_checkout_page_ie(self):
        ...
        cart_response = self.c.get(reverse('muecke_cart'))
        self.assertContains(cart_response, self.PRODUCT1_NAME, status_code=200)

        checkout_response = self.c.get(reverse('muecke_checkout'))
    ...
    def test_address_changed_on_checkout(self):
        ...
        checkout_post_response = self.c.post(reverse('muecke_checkout'), checkout_data)
        #self.dump_response(checkout_post_response)
        self.assertRedirects(checkout_post_response, reverse('muecke_thank_you'), status_code=302, target_status_code=200,)
    ...
    def test_ajax_saves_address(self):
        ...
        # register a new user
        registration_response = self.c.post(reverse('muecke_login'), {'action': 'register', 'email': 'test@test.com', 'password_1': 'password', 'password_2': 'password'})
        ...
        ajax_respons = self.c.post(reverse('muecke_changed_invoice_country'), form_data)
        ...
        ajax_respons = self.c.post(reverse('muecke_changed_shipping_country'), form_data)
        self.assertEquals(Address.objects.count(), 4)
