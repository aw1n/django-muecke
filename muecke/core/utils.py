...
def muecke_quote(string, encoding="utf-8"):
...
def set_message_to(response, msg):
    ...
    response.set_cookie("message", muecke_quote(msg), max_age=max_age, expires=expires)
    return response
...
def muecke_pagination(request, current_page, url='', getparam='start'):
